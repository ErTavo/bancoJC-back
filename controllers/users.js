...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    const [row] = await sql`
      INSERT INTO jc_users (username, password_hash, email)
      VALUES (${username}, ${encryptedPassword}, ${encryptedEmail})
      RETURNING id
    `;

    res.status(201).json({ id: row.id, message: 'Usuario creado exitosamente' });
  } catch (error) {
...
exports.getUserWithAuth = async (req, res) => {
  ...
  try {
    const rows = await sql`SELECT * FROM jc_users WHERE username = ${username}`;
    ...
    await sql`UPDATE jc_users SET last_hash = ${token} WHERE id = ${user.id}`;
...
exports.validateUserHash = async (req, res) => {
  ...
  try {
    ...
    const user = rows[0];
    if (user.last_hash !== hash) return res.status(401).json({ error: 'Hash no válido' });

    try {
      jwt.verify(hash, jwtSecret);
...
exports.sendEmailCode = async (req, res) => {
  ...
  try {
    ...
    await sql`UPDATE jc_users SET "lastOTP" = ${String(code)} WHERE id = ${user.id}`;
...
exports.validateOTP = async (req, res) => {
  ...
  try {
    ...
    const user = rows[0];
    if (user.lastOTP !== otp) return res.status(401).json({ error: 'Otp no válido' });
