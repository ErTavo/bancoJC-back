...
exports.createAccount = async (req, res) => {
  ...
  try {
    ...
    const encryptedBalance = encryptData(parseFloat(balance).toFixed(2), secretKey);
    if (!encryptedBalance) {
      return res.status(500).json({ error: 'Error al encriptar el balance' });
    }

    const [row] = await sql`
      INSERT INTO jc_accounts ("accountNo", "user", balance, type, currency)
      VALUES (${accountNo}, ${username}, ${encryptedBalance}, ${type}, ${currency})
      RETURNING id
    `;

    res.status(201).json({ id: row.id, accountNo, message: 'Cuenta creada exitosamente' });
  } catch (error) {
...
exports.balance = async (req, res) => {
  ...
  try {
    const rows = await sql`SELECT * FROM jc_accounts WHERE "user" = ${username}`;
    if (rows.length === 0) return res.status(404).json({ error: 'No hay cuentas disponibles' });
...
exports.deposito = async (req, res) => {
  ...
  try {
    ...
    const rows = await sql`SELECT * FROM jc_accounts WHERE "accountNo" = ${accountNo}`;
    if (rows.length === 0) return res.status(404).json({ error: 'No existe la cuenta indicada' });
    ...
    const tenis = Math.round((balance + abono) * 100) / 100;
    const encryptedBalance = encryptData(tenis.toFixed(2), secretKey);

    await sql`UPDATE jc_accounts SET balance = ${encryptedBalance} WHERE "accountNo" = ${accountNo}`;
